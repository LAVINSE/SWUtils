...
{
    /// <summary>
    /// SWUtilsPlayerPrefs 암호화 salt 설정 에셋을 생성하고 수정하는 에디터 창입니다.
    /// </summary>
    public class SWPlayerPrefsSettingsWindow : EditorWindow
    {
        private const string ResourcesFolderPath = "Assets/Resources";
        private const string SettingsAssetPath = ResourcesFolderPath + "/" + SWUtilsPlayerPrefsSettings.ResourceAssetName + ".asset";

        private SWUtilsPlayerPrefsSettings settings;
        private string salt;
        ...
        private void DrawAssetSection()
        {
            ...
            {
                EditorGUILayout.ObjectField("Current Asset", settings, typeof(SWUtilsPlayerPrefsSettings), false);
                EditorGUILayout.TextField("Runtime Path", "Resources/" + SWUtilsPlayerPrefsSettings.ResourceAssetName);
            }
        ...
        private void DrawSaltSection()
        {
            ...
            {
                salt = SWUtilsPlayerPrefsSettings.DefaultSalt;
                ivSalt = SWUtilsPlayerPrefsSettings.DefaultIVSalt;
            }
        ...
        private static void DrawWarningSection()
        {
            EditorGUILayout.HelpBox(
                "Salt 값을 변경하면 이전 salt로 저장된 SWUtilsPlayerPrefs 데이터는 읽을 수 없습니다. 변경 전에 데이터를 삭제하거나 별도 마이그레이션을 준비하세요.",
                MessageType.Warning);
        }
        ...
        private void LoadOrCreateSettings()
        {
            ...
            if (settings == null)
            {
                ...
                statusMessage = "Created SWUtilsPlayerPrefs settings asset.";
            }
            else
            {
                statusMessage = "Loaded existing SWUtilsPlayerPrefs settings asset.";
            }
        ...
        {
            salt = settings != null ? settings.Salt : SWUtilsPlayerPrefsSettings.DefaultSalt;
            ivSalt = settings != null ? settings.IVSalt : SWUtilsPlayerPrefsSettings.DefaultIVSalt;
        }
        ...
        private void SaveSettings()
        {
            ...
            Undo.RecordObject(settings, "Update SWUtilsPlayerPrefs Settings");
            settings.SetValues(salt, ivSalt);
            EditorUtility.SetDirty(settings);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            SWUtilsPlayerPrefs.ReloadSettings();

            PullValuesFromSettings();
            statusMessage = "Saved SWUtilsPlayerPrefs settings.";
        }

        private static SWUtilsPlayerPrefsSettings FindSettings()
        {
            string[] guids = AssetDatabase.FindAssets("t:SWUtilsPlayerPrefsSettings");
            foreach (string guid in guids)
            {
                string path = AssetDatabase.GUIDToAssetPath(guid);
                if (!IsResourcesPath(path)) continue;

                var asset = AssetDatabase.LoadAssetAtPath<SWUtilsPlayerPrefsSettings>(path);
                if (asset != null) return asset;
            }

            return AssetDatabase.LoadAssetAtPath<SWUtilsPlayerPrefsSettings>(SettingsAssetPath);
        }

        private static SWUtilsPlayerPrefsSettings CreateSettings()
        {
            EnsureResourcesFolder();

            var asset = CreateInstance<SWUtilsPlayerPrefsSettings>();
            asset.SetValues(SWUtilsPlayerPrefsSettings.DefaultSalt, SWUtilsPlayerPrefsSettings.DefaultIVSalt);
