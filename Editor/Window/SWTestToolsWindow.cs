    ...
    public class SWTestToolsWindow : EditorWindow
    {
        ...
        private void DrawPerformanceTab()
        {
            ...
            if (GUILayout.Button("GC.Collect 강제 실행"))
            {
                System.GC.Collect();
                cachedTotalMemoryMB = System.GC.GetTotalMemory(false) / (1024 * 1024);
                SWLog.Log("[SWTestTools] GC.Collect 실행됨");
            }
            EditorGUILayout.EndHorizontal();
        }
        ...
        private void BookmarkSelectedObject()
        {
            ...
            {
                SWLog.Log($"[SWTestTools] 이미 북마크된 GameObject입니다: {hierarchyPath}");
                return;
            }
        ...
        private void SelectBookmark(string scenePath, string hierarchyPath)
        {
            ...
            if (currentActive.path != scenePath && !string.IsNullOrEmpty(scenePath))
            {
                ...
                {
                    SWLog.LogWarning($"[SWTestTools] 씬 파일이 없습니다: {scenePath}");
                    return;
                }
            }
            ...
            {
                SWLog.LogWarning($"[SWTestTools] GameObject를 찾을 수 없습니다: {hierarchyPath}");
            }
        }
        ...
        private void RegisterScene(string path)
        {
            ...
            {
                SWLog.Log($"[SWTestTools] 이미 등록된 씬입니다: {path}");
                return;
            }
        ...
        {
            SWEditorUtils.DrawHeader("SWPlayerPrefs");
            if (SWEditorUtils.DangerButton("SWPlayerPrefs.DeleteAll", "SWPlayerPrefs 삭제",
                "현재 슬롯의 모든 SWPlayerPrefs 데이터를 삭제하시겠습니까?", "삭제", "취소"))
            {
                SWPlayerPrefs.DeleteAll();
                SWLog.Log("[SWTestTools] SWPlayerPrefs 전체 삭제됨");
            }
