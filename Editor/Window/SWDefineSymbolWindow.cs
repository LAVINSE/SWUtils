...
namespace SW.EditorTools.Window
{
    ...
    public class SWDefineSymbolWindow : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            SWLog.Log("[SWDefineSymbolWindow] Opened.");
        }

        private void OnDisable()
        {
            SavePresets();
            SWLog.Log("[SWDefineSymbolWindow] Closed.");
        }
        #endregion // 초기화
        ...
        private void DrawTargetSection()
        {
                    ...
                    $"Remove all define symbols from {selectedGroup}?", "Clear"))
            {
                ...
                ApplySymbols();
                SWLog.LogWarning($"[SWDefineSymbolWindow] Clear all defines. Group: {selectedGroup}");
            }
        ...
        private void DrawPresetSection()
        {
            ...
            for (int index = 0; index < presetSymbols.Count; index++)
            {
                ...
                if (GUILayout.Button("Remove Preset", GUILayout.Width(110)))
                {
                    ...
                    SavePresets();
                    SWLog.Log($"[SWDefineSymbolWindow] Remove preset: {symbol}");
                    GUIUtility.ExitGUI();
                }
                EditorGUILayout.EndHorizontal();
            }
            ...
            using (new SWEditorUtils.GUIEnabledScope(IsValidSymbol(presetInput) && !presetSymbols.Contains(presetInput)))
            {
                if (GUILayout.Button("Add Preset", GUILayout.Width(90)))
                {
                    ...
                    presetInput = "";
                    SavePresets();
                    SWLog.Log($"[SWDefineSymbolWindow] Add preset: {symbol}");
                    GUI.FocusControl(null);
                }
            }
            EditorGUILayout.EndHorizontal();
        ...
        private void RefreshSymbols()
        {
            ...
            SWLog.Log($"[SWDefineSymbolWindow] Refresh. Group: {selectedGroup}, Count: {currentSymbols.Count}");
        }
        ...
        private void AddSymbol(string symbol)
        {
            ...
            {
                SWLog.LogWarning($"[SWDefineSymbolWindow] Add symbol failed. Invalid symbol: {symbol}");
                return;
            }
            ...
            SWLog.Log($"[SWDefineSymbolWindow] Add symbol: {symbol}");
        }
        ...
        private void RemoveSymbol(string symbol)
        {
            ...
            ApplySymbols();
            SWLog.Log($"[SWDefineSymbolWindow] Remove symbol: {symbol}");
        }
        ...
        private void ApplySymbols()
        {
            ...
            SWLog.Log($"[SWDefineSymbolWindow] Apply symbols. Group: {selectedGroup}, Symbols: {defineSymbols}");
        }
