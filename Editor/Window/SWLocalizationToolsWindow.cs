    ...
    public class SWLocalizationToolsWindow : EditorWindow
    {
        ...
        private void ExportCollection()
        {
            ...
            try
            {
                ...
                SWLog.Log($"[SWLocalization] Export 완료: {fullPath}");
            }
            catch (Exception e)
            {
                EditorUtility.DisplayDialog("오류", $"내보내기 중 오류가 발생했습니다:\n{e.Message}", "확인");
                SWLog.LogError($"[SWLocalization] Export 실패: {e}");
            }
        }
        ...
        private void PreviewImportData()
        {
            try
            {
                ...
                SWLog.Log($"[SWLocalization] 미리보기: {importPreviewEntries.Count}개 항목 파싱됨");
            }
            catch (Exception e)
            {
        ...
        private void GenerateLocalizationTables()
        {
            try
            {
                ...
                foreach (var entry in entries)
                {
                    ...
                    foreach (var locale in selectedLocales)
                    {
                        ...
                        if (table == null)
                        {
                            ...
                            {
                                SWLog.LogWarning($"[SWLocalization] 로케일 '{locale.LocaleName}' 테이블 생성 실패: {e.Message}");
                                continue;
                            }
                        }
                ...
                SWLog.Log($"[SWLocalization] '{collectionName}' {actionText} 완료: {entries.Count}개 항목");
            }
            catch (Exception e)
            {
                EditorUtility.ClearProgressBar();
                EditorUtility.DisplayDialog("오류", $"Localization Table 생성 중 오류:\n{e.Message}", "확인");
                SWLog.LogError($"[SWLocalization] Import 실패: {e}");
            }
        }
        ...
        private void ClearAllKeysFromCollection(StringTableCollection collection)
        {
            ...
            SWLog.Log($"[SWLocalization] '{collection.name}'에서 {keysToRemove.Count}개 키가 삭제되었습니다.");
        }
        ...
        private void RemoveKeysNotInTSV(StringTableCollection collection, HashSet<string> tsvKeys)
        {
            ...
            SWLog.Log($"[SWLocalization] '{collection.name}'에서 TSV에 없는 {keysToRemove.Count}개 키가 삭제되었습니다.");
        }
        ...
        private StringTableCollection GetOrCreateStringTableCollection(string name)
        {
            ...
            foreach (var locale in selectedLocales)
            {
                ...
                {
                    SWLog.LogWarning($"[SWLocalization] 로케일 '{locale.LocaleName}' 테이블 추가 실패: {e.Message}");
                }
            }
        ...
        private int ApplySmartStringToKeys(List<string> keys, bool enableSmart)
        {
            ...
            try
            {
                ...
                SWLog.Log($"[SWLocalization] Smart String {(enableSmart ? "ON" : "OFF")}: {processed}개 항목 처리됨");
            }
            finally
            {
                EditorUtility.ClearProgressBar();
        ...
        private void ValidateCollection()
        {
            ...
            SWLog.Log($"[SWLocalization] '{selectedCollection.name}' 검증 완료: {validationResults.Count}개 결과");
        }
        ...
        private void ValidateAllCollections()
        {
            ...
            SWLog.Log($"[SWLocalization] 전체 검증 완료: {validationResults.Count}개 결과");
        }
