...
namespace SW.EditorTools.Data
{
    ...
    public static class SWExcelTableParser
    {
        ...
        /// <summary>
        /// 각 행이 필드명과 값으로 구성된 세로형 TSV 문자열을 단일 행 데이터로 파싱합니다.
        /// </summary>
        ...
        #region 시트 필드
        /// <summary>
        /// ScriptableObject에서 SWTableSheetAttribute가 붙은 리스트/배열 필드를 찾습니다.
        /// </summary>
        ...
        public static List<SheetFieldInfo> GetSheetFields(ScriptableObject target)
        {
            ...
            foreach (FieldInfo field in fields)
            {
                ...
                {
                    SWLog.LogWarning($"[SWExcelTableParser] 지원하지 않는 시트 필드 타입입니다. Field: {field.Name}");
                }
            }
        ...
        /// <summary>
        /// 리스트, 배열 또는 단일 클래스 필드에서 매핑할 데이터 타입을 가져옵니다.
        /// </summary>
        ...
        public static bool ApplyToSheet(ScriptableObject target, SheetFieldInfo sheetField, ParseResult parseResult)
        {
            ...
            SWLog.Log($"[SWExcelTableParser] Apply complete. Sheet: {sheetField.SheetName}, Count: {values.Count}");
            return true;
        }
