...
namespace SW.Util
{
    ...
    public class SWRefillTimer
    {
        ...
        public string RemainText => SWTime.ToMinSec(RemainSeconds);

        /// <summary>다음 1개 회복까지 남은 시간을 자연어로 반환.</summary>
        public string RemainReadable => SWTime.ToReadable(RemainSeconds);
        ...
        public string RemainTextAll => SWTime.ToHourMinSec(RemainSecondsAll);
        ...
        /// <summary>
        /// 지정한 초기 수량과 마지막 사용 시각으로 SWRefillTimer를 생성합니다.
        /// 서버 동기화 데이터처럼 외부 상태를 기준으로 시작할 때 사용합니다.
        /// </summary>
        /// <param name="id">고유 식별자 (PlayerPrefs 키에 사용)</param>
        /// <param name="maxCount">최대 보유 수량</param>
        /// <param name="intervalSeconds">1개 회복 간격(초)</param>
        ...
        #region 기능
        /// <summary>
        /// 경과 시간에 따라 회복량을 계산하고 수량을 갱신합니다.
        /// Update 또는 앱 복귀 시 호출합니다.
        /// </summary>
        ...
        #region 저장 / 로드
        /// <summary>
        /// 현재 상태를 PlayerPrefs에 저장합니다.
        /// </summary>
        private void Save()
        {
            SWPlayerPrefs.SetInt(KeyCount, count);
            SWPlayerPrefs.SetString(KeyLastUse, lastUseUtc.ToString("o"));
            SWPlayerPrefs.Save();
        }
        ...
        private void Load()
        {
            if (!SWPlayerPrefs.HasKey(KeyCount)) return;

            count = Mathf.Clamp(SWPlayerPrefs.GetInt(KeyCount, maxCount), 0, maxCount);

            string stored = SWPlayerPrefs.GetString(KeyLastUse, "");
        ...
        /// <summary>
        /// 저장 데이터를 초기화하고 수량을 최대치로 재설정합니다.
        /// </summary>
        public void Reset()
        {
            count = maxCount;
            lastUseUtc = DateTime.UtcNow;
            SWPlayerPrefs.DeleteKey(KeyCount);
            SWPlayerPrefs.DeleteKey(KeyLastUse);
            SWPlayerPrefs.Save();
        }
        #endregion // 저장 / 로드
    }
