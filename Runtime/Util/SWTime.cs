...
namespace SW.Util
{
    /// <summary>
    /// 시간 처리와 관련된 통합 유틸리티.
    /// 서버/클라이언트 시간 보정, 파싱, 포맷팅, 쿨다운, 오프라인 시간 처리 등을 제공합니다.
    /// </summary>
    public class SWTime
    {
        #region 필드
        ...
        /// <summary>
        /// 서버 응답 시각과 로컬 시각을 비교하여 오프셋을 갱신합니다.
        /// </summary>
        ...
        #region 파싱 & 변환
        /// <summary>
        /// 서버에서 내려온 시간 문자열을 <see cref="DateTime"/>(UTC)으로 안전하게 파싱합니다.
        /// </summary>
        ...
        /// <summary>
        /// 시간 문자열을 <see cref="DateTime"/>으로 변환합니다.
        /// 문자열이 비어 있으면 현재 로컬 시각을 반환합니다.
        /// </summary>
        ...
        /// <summary>
        /// <see cref="DateTime"/>을 지정한 포맷의 문자열로 변환합니다.
        /// </summary>
        ...
        /// <summary>
        /// "yyMMdd" 축약 날짜 문자열을 "yyyy-MM-dd" 형식으로 변환합니다.
        /// </summary>
        ...
        #region Unix 타임스탬프
        /// <summary>
        /// Unix 타임스탬프(초)를 <see cref="DateTime"/>(UTC)으로 변환합니다.
        /// </summary>
        ...
        /// <summary>
        /// <see cref="DateTime"/>(UTC)을 Unix 타임스탬프(초)로 변환합니다.
        /// </summary>
        ...
        #region 경과 / 남은 시간 계산
        /// <summary>
        /// 서버 기준 경과 시간(초)을 반환합니다. 양수=시간 지남, 음수=아직 안 됨.
        /// </summary>
        ...
        /// <summary>
        /// 서버 기준 남은 시간(초)을 반환합니다. 양수=아직 남음, 음수=이미 지남.
        /// </summary>
        ...
        /// <summary>
        /// 현재 시각으로부터 특정 시간 후의 <see cref="DateTime"/>을 반환합니다.
        /// </summary>
        ...
        /// <summary>
        /// 오늘 자정(00:00) 기준 특정 시각의 <see cref="DateTime"/>을 반환합니다.
        /// </summary>
        ...
        /// <summary>
        /// 미래 시점까지 남은 시간을 상대 시간 문자열로 변환합니다.
        /// </summary>
        ...
        /// <summary>
        /// <see cref="DateTime"/> 기반 쿨다운 완료 여부를 확인합니다.
        /// </summary>
        ...
        /// <summary>
        /// <see cref="DateTime"/> 기반 남은 쿨다운 시간(초)을 반환합니다.
        /// </summary>
        /// <param name="startUtc">쿨다운 시작 UTC 시각</param>
        /// <param name="durationSeconds">쿨다운 지속 시간(초)</param>
        /// <returns>남은 시간(초). 최소 0</returns>
        ...
        /// <summary>
        /// 오프라인 경과 시간을 초 단위 상한선을 적용하여 반환합니다.
        /// </summary>
        ...
        /// <summary>
        /// 오프라인 경과 시간을 일/시/분/초 조합 상한선을 적용하여 반환합니다.
        /// </summary>
        ...
        /// <summary>
        /// 다음 특정 리셋 시각(UTC)까지 남은 초를 반환합니다.
        /// </summary>
        ...
        /// <summary>
        /// 오늘(UTC) 이미 리셋되었는지 확인합니다. 자정(00:00) 기준.
        /// </summary>
        /// <param name="key">PlayerPrefs 키</param>
        /// <returns>리셋 완료이면 true</returns>
        public static bool HasResetToday(string key = DefaultResetKey)
            => SWPlayerPrefs.GetString(key, "") == DateTime.UtcNow.ToString("yyyy-MM-dd");

        /// <summary>
        /// 커스텀 리셋 시각(UTC) 기준으로 현재 리셋 주기에서 이미 리셋되었는지 확인합니다.
        /// </summary>
        /// <param name="resetHourUtc">리셋 기준 시(hour, UTC)</param>
        /// <param name="key">PlayerPrefs 키</param>
        /// <returns>리셋 완료이면 true</returns>
        public static bool HasResetToday(int resetHourUtc, string key = DefaultResetKey)
            => SWPlayerPrefs.GetString(key, "") == GetResetDateKey(resetHourUtc);

        /// <summary>
        /// 리셋 완료로 기록합니다. 자정(00:00) 기준.
        /// </summary>
        /// <param name="key">PlayerPrefs 키</param>
        public static void MarkResetDone(string key = DefaultResetKey)
        {
            SWPlayerPrefs.SetString(key, DateTime.UtcNow.ToString("yyyy-MM-dd"));
            SWPlayerPrefs.Save();
        }

        /// <summary>
        /// 커스텀 리셋 시각(UTC) 기준으로 리셋 완료를 기록합니다.
        /// </summary>
        /// <param name="resetHourUtc">리셋 기준 시(hour, UTC)</param>
        /// <param name="key">PlayerPrefs 키</param>
        public static void MarkResetDone(int resetHourUtc, string key = DefaultResetKey)
        {
            SWPlayerPrefs.SetString(key, GetResetDateKey(resetHourUtc));
            SWPlayerPrefs.Save();
        }

        /// <summary>
        /// 리셋이 필요하면 true를 반환하고 자동으로 기록을 갱신합니다. 자정(00:00) 기준.
        /// </summary>
        /// <param name="key">PlayerPrefs 키</param>
        ...
        /// <summary>
        /// 커스텀 리셋 시각(UTC) 기준으로 리셋이 필요하면 true를 반환하고 자동으로 기록을 갱신합니다.
        /// </summary>
        /// <param name="resetHourUtc">리셋 기준 시(hour, UTC)</param>
        /// <param name="key">PlayerPrefs 키</param>
        /// <returns>리셋이 수행되었으면 true</returns>
        ...
        /// <summary>
        /// 커스텀 리셋 시각 기준의 현재 리셋 주기 날짜 키를 반환합니다.
        ...
        /// <summary>
        /// 스톱워치를 정지하고 경과 시간(ms)을 반환합니다.
        /// SW_DEBUG_MODE가 아니면 항상 0을 반환합니다.
        /// </summary>
        ...
        /// <summary>
        /// 스톱워치를 정지하지 않고 현재까지의 경과 시간(ms)을 반환합니다.
        /// SW_DEBUG_MODE가 아니면 항상 0을 반환합니다.
        /// </summary>
        /// <returns>경과 시간(밀리초)</returns>
