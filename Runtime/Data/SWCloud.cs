...
{
    /// <summary>
    /// 플랫폼별 클라우드 저장소를 하나의 인터페이스로 제공합니다.
    /// </summary>
    /// <remarks>
    /// 지원되는 플랫폼 서비스가 없거나 에디터에서 실행 중이면 로컬 플레이어 설정 저장소를 사용합니다.
    /// </remarks>
    public static class SWCloud
    {
        #region 필드
        /// <summary>기본 저장 슬롯 이름.</summary>
        public const string DefaultSaveName = SWSaveSlot.Default;
        /// <summary>로컬 폴백 PlayerPrefs 키 접두사.</summary>
        ...
        /// <summary>
        /// iCloud에서 Base64로 디코딩된 문자열을 가져옵니다.
        /// </summary>
        ...
        private static string iCloudGet(string key)
        {
            ...
            {
                SWLog.LogError($"[SWCloud] iCloud Base64 디코딩 실패: {exception.Message}");
                return string.Empty;
            }
        }

        /// <summary>
        /// iCloud에 Base64로 인코딩하여 저장합니다.
        /// </summary>
        /// <param name="key">저장 키</param>
        ...
        private static bool iCloudSet(string key, string value)
        {
            ...
            {
                SWLog.LogError($"[SWCloud] iCloud Base64 인코딩 실패: {exception.Message}");
                return false;
            }
        }
#endif
        ...
        public static void Init(Action<bool> onComplete = null)
        {
            ...
            isInitialized = true;
            SWLog.Log("[SWCloud] iCloud 초기화 완료");
            onComplete?.Invoke(true);
#elif (UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX) && SW_STEAMWORKS_NET && !UNITY_EDITOR
            isAuthenticated = SteamManager.Initialized;
            isInitialized = true;
            SWLog.Log($"[SWCloud] Steam 초기화 상태: {isAuthenticated}");
            onComplete?.Invoke(isAuthenticated);
#else
            isInitialized = true;
            isAuthenticated = false;
            SWLog.LogWarning("[SWCloud] 활성화된 클라우드 SDK가 없어 로컬 fallback을 사용합니다.");
            onComplete?.Invoke(false);
#endif
        }

        /// <summary>
        /// 클라우드 시스템을 초기화하고 인증을 시도합니다. (비동기)
        /// </summary>
        ...
        public static void Save(string json, Action<bool> onComplete = null, string saveName = DefaultSaveName)
        {
            ...
            {
                SWLog.LogError("[SWCloud] 저장 실패: JSON 문자열이 비어 있습니다.");
                onComplete?.Invoke(false);
                return;
            }

            SWLog.Log($"[SWCloud] 저장 시작 (길이: {json.Length}, 제공자: {ProviderName})");

#if UNITY_ANDROID && SW_GOOGLEPLAY_ENABLE && !UNITY_EDITOR
            if (!isAuthenticated)
            {
                SWLog.LogWarning("[SWCloud] GPGS 인증이 없어 로컬에만 저장합니다.");
            ...
            PlayGamesPlatform.Instance.SavedGame.OpenWithAutomaticConflictResolution(
                ...
                (status, metadata) =>
                {
                    ...
                    {
                        SWLog.LogError($"[SWCloud] GPGS 저장 데이터 열기 실패: {status}");
                        SaveLocalAndComplete(json, saveName, onComplete, false);
                        return;
                    }
                    ...
                    PlayGamesPlatform.Instance.SavedGame.CommitUpdate(metadata, update, data, (commitStatus, _) =>
                    {
                        ...
                        SWLog.Log($"[SWCloud] GPGS 저장 커밋 결과: {commitStatus}");
            ...
            try
            {
                ...
                _ForceSynciCloudData();
                SWLog.Log($"[SWCloud] iCloud 저장 결과: {result}");
                SaveLocalAndComplete(json, saveName, onComplete, result);
            }
            catch (Exception exception)
            {
                SWLog.LogError($"[SWCloud] iCloud 저장 실패: {exception.Message}");
                SaveLocalAndComplete(json, saveName, onComplete, false);
            }
#elif (UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX) && SW_STEAMWORKS_NET && !UNITY_EDITOR
            if (!SteamManager.Initialized)
            {
                SWLog.LogWarning("[SWCloud] Steam이 초기화되지 않아 로컬에만 저장합니다.");
                SaveLocalAndComplete(json, saveName, onComplete, false);
                return;
            }
            ...
            try
            {
                ...
                SWLog.Log($"[SWCloud] Steam 저장 결과: {result}");
                SaveLocalAndComplete(json, saveName, onComplete, result);
            }
            catch (Exception exception)
            {
                SWLog.LogError($"[SWCloud] Steam 저장 실패: {exception.Message}");
                SaveLocalAndComplete(json, saveName, onComplete, false);
            }
#else
            SWLog.Log("[SWCloud] 로컬 fallback 저장");
            SaveLocalAndComplete(json, saveName, onComplete, true);
#endif
        }

        /// <summary>
        /// JSON 문자열을 클라우드에 저장합니다. (비동기)
        /// </summary>
        /// <param name="json">저장할 JSON 문자열</param>
        /// <param name="saveName">저장 슬롯 이름</param>
        /// <returns>성공 여부</returns>
        ...
        {
            saveName = NormalizeSaveName(saveName);
            SWLog.Log($"[SWCloud] 로드 시작 (제공자: {ProviderName})");

#if UNITY_ANDROID && SW_GOOGLEPLAY_ENABLE && !UNITY_EDITOR
            if (!isAuthenticated)
            {
                SWLog.LogWarning("[SWCloud] GPGS 인증이 없어 로컬에서만 로드합니다.");
            ...
            PlayGamesPlatform.Instance.SavedGame.OpenWithAutomaticConflictResolution(
                ...
                (status, metadata) =>
                {
                    ...
                    PlayGamesPlatform.Instance.SavedGame.ReadBinaryData(metadata, (readStatus, data) =>
                    {
                        ...
                        {
                            SWLog.LogWarning($"[SWCloud] GPGS 읽기 결과가 비어 있습니다: {readStatus}");
                            LoadLocalAndComplete(saveName, onComplete);
                            return;
                        }

                        string json = Encoding.UTF8.GetString(data);
                        SWLog.Log($"[SWCloud] GPGS 로드 성공 (길이: {json.Length})");
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(json))
                {
                    SWLog.LogWarning("[SWCloud] iCloud 데이터가 비어 있어 로컬 fallback으로 로드합니다.");
                    LoadLocalAndComplete(saveName, onComplete);
                }
                else
                {
                    SWLog.Log($"[SWCloud] iCloud 로드 성공 (길이: {json.Length})");
                    CacheLocalAndComplete(json, saveName, onComplete, true);
                }
            }
            catch (Exception exception)
            {
                SWLog.LogError($"[SWCloud] iCloud 로드 실패: {exception.Message}");
                LoadLocalAndComplete(saveName, onComplete);
            }
#elif (UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX) && SW_STEAMWORKS_NET && !UNITY_EDITOR
            if (!SteamManager.Initialized)
            {
                SWLog.LogWarning("[SWCloud] Steam이 초기화되지 않아 로컬에서만 로드합니다.");
                LoadLocalAndComplete(saveName, onComplete);
                return;
            }
            ...
            try
            {
                ...
                {
                    SWLog.LogWarning("[SWCloud] Steam 파일을 찾을 수 없어 로컬 fallback으로 로드합니다.");
                    LoadLocalAndComplete(saveName, onComplete);
                    return;
                }
                ...
                if (read <= 0)
                {
                    SWLog.LogWarning("[SWCloud] Steam 읽기 결과가 비어 있어 로컬 fallback으로 로드합니다.");
                    LoadLocalAndComplete(saveName, onComplete);
                    return;
                }
                ...
                SWLog.Log($"[SWCloud] Steam 로드 성공 (길이: {json.Length})");
                CacheLocalAndComplete(json, saveName, onComplete, true);
            }
            catch (Exception exception)
            {
                SWLog.LogError($"[SWCloud] Steam 로드 실패: {exception.Message}");
                LoadLocalAndComplete(saveName, onComplete);
            }
#else
            LoadLocalAndComplete(saveName, onComplete);
        ...
        {
            saveName = NormalizeSaveName(saveName);
            SWLog.Log($"[SWCloud] 삭제 시작 (제공자: {ProviderName})");
            ...
            PlayGamesPlatform.Instance.SavedGame.OpenWithAutomaticConflictResolution(
                ...
                (status, metadata) =>
                {
                    if (status == SavedGameRequestStatus.Success)
                    {
                        ...
                        DeleteLocal(saveName);
                        SWLog.Log("[SWCloud] GPGS 삭제 성공");
                        onComplete?.Invoke(true);
                    }
                    else
                    {
                        SWLog.LogError($"[SWCloud] GPGS 삭제 실패: {status}");
                        onComplete?.Invoke(false);
                    }
                });
#elif UNITY_IOS && !UNITY_EDITOR
            try
            {
                ...
                _ForceSynciCloudData();
                DeleteLocal(saveName);
                SWLog.Log($"[SWCloud] iCloud 삭제 결과: {result}");
                onComplete?.Invoke(result);
            }
            catch (Exception exception)
            {
                SWLog.LogError($"[SWCloud] iCloud 삭제 실패: {exception.Message}");
                onComplete?.Invoke(false);
            }
#elif (UNITY_STANDALONE_WIN || UNITY_STANDALONE_OSX || UNITY_STANDALONE_LINUX) && SW_STEAMWORKS_NET && !UNITY_EDITOR
            try
            {
                ...
                DeleteLocal(saveName);
                SWLog.Log($"[SWCloud] Steam 삭제 결과: {result}");
                onComplete?.Invoke(result);
            }
            catch (Exception exception)
            {
                SWLog.LogError($"[SWCloud] Steam 삭제 실패: {exception.Message}");
                onComplete?.Invoke(false);
            }
#else
            DeleteLocal(saveName);
            SWLog.Log("[SWCloud] 로컬 fallback 삭제");
            onComplete?.Invoke(true);
#endif
        }

        /// <summary>
        /// 클라우드에서 저장 데이터를 삭제합니다. (비동기)
        /// </summary>
        /// <param name="saveName">저장 슬롯 이름</param>
        /// <returns>성공 여부</returns>
        ...
        /// <summary>
        /// 객체를 JSON으로 직렬화하여 클라우드에 저장합니다. (비동기)
        /// </summary>
        public static Task<bool> SaveJsonAsync<T>(T data, string saveName = DefaultSaveName)
        {
            if (data == null)
            {
                SWLog.LogError("[SWCloud] SaveJsonAsync 실패: data가 null입니다.");
        ...
        /// <summary>
        /// 클라우드에서 JSON을 로드하여 객체로 역직렬화합니다. (콜백)
        /// </summary>
        public static void LoadJson<T>(Action<bool, T> onComplete, string saveName = DefaultSaveName) where T : class
        {
            Load((success, json) =>
            {
                ...
                {
                    SWLog.LogError($"[SWCloud] LoadJson 실패: {exception.Message}");
                    onComplete?.Invoke(false, default);
                }
            }, saveName);
        }
        ...
        /// <summary>
        /// 클라우드에서 JSON을 로드하여 객체로 역직렬화합니다. (비동기)
        /// </summary>
        public static async Task<(bool success, T data)> LoadJsonAsync<T>(string saveName = DefaultSaveName) where T : class
        {
            ...
            {
                SWLog.LogError($"[SWCloud] LoadJsonAsync 실패: {exception.Message}");
        ...
        #region 로컬 폴백
        /// <summary>
        /// 비어 있거나 공백인 저장 이름을 기본값으로 정규화합니다.
        /// </summary>
        ...
        /// <summary>
        /// 로컬에 저장하고 Action&lt;bool&gt; 콜백을 호출합니다.
        /// </summary>
        ...
        /// <summary>
        /// 클라우드 로드 성공 시 로컬 캐시를 갱신하고 Action&lt;bool, string&gt; 콜백을 호출합니다.
        /// </summary>
        ...
        #region SWPlayerPrefs 통합
        /// <summary>
        /// SWPlayerPrefs의 모든 데이터를 클라우드에 백업합니다. (콜백)
        /// </summary>
        /// <param name="onComplete">완료 콜백 (성공 여부)</param>
        /// <param name="saveName">저장 슬롯 이름</param>
        ...
        /// <summary>
        /// SWPlayerPrefs의 모든 데이터를 클라우드에 백업합니다. (비동기)
        /// </summary>
        /// <param name="saveName">저장 슬롯 이름</param>
        /// <returns>성공 여부</returns>
        ...
        /// <summary>
        /// 클라우드에서 데이터를 받아 SWPlayerPrefs로 복원합니다. (콜백)
        /// </summary>
        /// <param name="onComplete">완료 콜백 (성공 여부)</param>
        ...
        /// <summary>
        /// 클라우드에서 데이터를 받아 SWPlayerPrefs로 복원합니다. (비동기)
        /// </summary>
        /// <param name="merge">true면 병합, false면 전체 덮어쓰기</param>
        /// <param name="saveName">저장 슬롯 이름</param>
        /// <returns>성공 여부</returns>
