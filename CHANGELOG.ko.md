...
| 최신 버전 | 날짜 | 핵심 내용 |
| --- | --- | --- |
| `v1.3.0` | 2026-09-10 | 인크리멘탈 스킬트리, 81개 노드 예제와 제작 편집기 개선 |
...
- 인크리멘탈 강화, 연구 해금과 특성 선택을 위한 `SW.SkillTree` 모듈을 추가했습니다. 공용 정의와 소유자별 진행을 분리하고 비용, 조건, 효과, 재화와 저장소를 확장할 수 있습니다.
- 선행 레벨, 모든 조건·하나 이상 조건, 상호 배타 분기, 반복 강화, 여러 재화 구매, 일괄 구매, 원지불 비용 환불, 저장·복원과 영구 노드를 유지하는 진행 초기화를 지원합니다.
- Skill Tree Editor와 게임 화면을 추가했습니다. 실제 노드 위치에 맞춰 연결선을 갱신하고, 수정한 배치를 공용 트리 에셋에 저장하여 제작 편집기와 공유합니다. 자동 배치는 저장 위치가 없는 노드에 적용합니다.
- 선행 진행에 따른 노드 숨김·정보 가리기와 편집 모드 전용 전체 노드 보기를 추가했습니다. 플레이어 진행 저장에는 배치 데이터를 포함하지 않습니다.
- 81개 노드와 6개 확장 경로의 MiningSkillTree 예제, 노드 프리팹, 구매·환불 및 저장·복원 조작을 추가했습니다. 드래그 이동, 포인터 중심 확대, 시작·선택 노드 이동과 전체 보기를 지원합니다.
- 스킬트리 사용법과 확장 계약, 편집 모드 테스트를 추가했습니다.

### 변경

- `SWIdentifiedObject` 공통 필드를 기본으로 접힌 **기본 정의** 폴드아웃에 묶었습니다. 기존 에셋과 파생 타입에도 적용하며 사용자의 펼침 상태를 유지합니다.
.
- 스킬트리 화면은 TreeView에서 노드 크기를 직접 지정하고, 별도의 ViewStyle 에셋 없이 TextMeshProUGUI와 프로젝트 기본 글꼴을 사용합니다. 별도 글꼴 데이터는 포함하지 않습니다.
- 예제 생성·재생성을 SWTools 메뉴에 노출하지 않고 인스펙터 배치 작업과 편집기 코드로 제공합니다. 노드의 유지 설정은 **진행 초기화 시 유지**로 표시합니다.

## [v1.2.2] - 2026-09-07

### 수정

- Unity 6.4 이상에서 오브젝트 조회, 하이어라키 캐시와 콜백, 그래프 에셋 열기에 EntityId를 사용하도록 수정했습니다. 이전 버전은 기존 식별자 방식을 유지합니다.
- 풀 모니터의 펼침 상태를 EntityId 배열로 저장하고, 인스펙터 설정 키에서 64비트 식별자가 손실되지 않도록 수정했습니다.
- 하이어라키 재귀 펼침 호출의 매개변수 타입을 Unity 버전에 맞추고, 교차 배경을 식별자 값 대신 행 위치로 계산하도록 수정했습니다.
- Unity 6.6에서 사용이 금지된 드롭다운 children 속성을 childList로 교체하고, 최신 버전의 오브젝트 검색에서 폐기된 정렬 인자를 제거했습니다.

### 검증

- Unity 6000.6.0f1과 6000.3.11f1 어셈블리를 참조해 Runtime, Editor, Samples 전체 스크립트의 컴파일을 확인했습니다.

## [v1.2.1] - 2026-09-07

### 수정

- 전체 스크립트를 점검하여 SWTimer, SWCooldown, SWSaveDataManager의 Fields, Properties 등 일반 코드 구획 이름 15개를 기존 한글 표기에 맞췄습니다.
- 영역 이름에 대응하는 종료 주석도 함께 수정했습니다. 실행 코드의 동작 변경은 없습니다.
- 모호한 보조 기능 명칭과 코루틴 설명을 실제 동작에 맞게 다듬고, 중복 주석과 과거 변경 이력을 정리했습니다.
- SWTimer.Reset이 일시정지 상태도 초기화한다는 설명을 보완했습니다.

## [v1.2.0] - 2026-09-07

### 추가
- 작업 묶음, 조건, 보상과 진행 보고를 조합하는 `SW.Quest` 퀘스트 및 업적 모듈을 추가했습니다.
- 순차 작업 묶음, 카테고리와 대상 기반 진행 보고, 교체·누적·양수·음수·연속 진행 전략을 추가했습니다.
- 확장 가능한 수락·취소 조건과 보상, 업적 자동 등록, 중복 방지, 지역 이벤트와 `SWEventBus` 이벤트를 추가했습니다.
- 모든 작업 묶음 진행 상태를 보존하는 `SWQuestSystemSaveData`, 교체 가능한 `ISWQuestSaveStore`와 암호화된 `SWPlayerPrefs` 기본 저장소를 추가했습니다.
- 일반 퀘스트와 업적 데이터베이스를 분리하고 각각의 자동 수집·검증 기능을 추가했습니다.
- 퀘스트 관련 에셋의 생성, 복제, 삭제, 검색, 편집과 데이터베이스 관리를 제공하는 `SWQuestSystemWindow`를 추가했습니다.
- 프로젝트별 점수 보상 샘플을 추가했습니다.

### 수정
- 음수 진행 보고가 진행량을 증가시키지 않고 올바르게 감소시키도록 수정했습니다.
- 작업 묶음과 작업을 코드명으로 복원하여 순서 변경 시 진행량이 다른 정의에 적용되지 않게 하고, 완료 보상 중복 지급과 새 작업 비활성 문제를 방지했습니다.

### 주의
- 퀘스트와 업적 시스템은 전체 설계 검토와 실제 프로젝트 검증이 끝나지 않은 실험적 기능입니다.
- 추후 검토 결과에 따라 공개 기능과 저장 데이터 형식이 변경될 수 있습니다.
