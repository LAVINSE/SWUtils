...
| 기본 본문·보조 글자 | 14·12 픽셀 |
| 버튼·입력 영역 최소 높이 | 32 픽셀 |
| 탭 닫기 영역 | 28 × 28 픽셀 |

분류와 목록의 선택 항목에는 왼쪽 강조선을, 활성 탭에는 아래쪽 강조선을 표시합니다. 주요 생성 버튼은 밝은 강조색 바탕과 어두운 글자로 구분합니다. 열린 비활성 에셋과 마우스를 올린 항목도 같은 회보라색 계열을 사용합니다. 작업 공간 안의 기본 그룹 강조색은 공통 테마를 따르며, 어트리뷰트에 직접 지정한 색상과 그래프의 실행·성공·실패 상태 색상은 유지합니다.

작은 그래프 경로와 상태 표식은 12픽셀로 맞춥니다. 그래프 도구 모음은 42픽셀로 늘려 32픽셀 버튼을 수용합니다. 위 표의 입력 영역 높이는 UI Toolkit 기준이며, 고정 사각형을 사용하는 기존 편집기에서는 호출자가 정한 높이를 유지합니다.

`Editor/StyleSheet/SWEditorThemeStylesheet.uss`와 `Editor/Theme/SWEditorTheme.cs`가 공통 컨트롤 디자인을 담당합니다. UI Toolkit은 루트에 스타일을 적용하고, IMGUI 편집기 창은 그리는 범위를 `SWEditorThemeScope`로 감싼 뒤 원래 스타일을 복원합니다.

`SWMonoBehaviour`와 `SWScriptableObject`의 일반 인스펙터에는 `SWEditorTheme.ApplyInspector`로 Iris Line을 적용합니다. 전용 색상과 크기는 `Editor/StyleSheet/SWMonobehaviourEditorStylesheet.uss`의 `.sw-inspector-theme`에서 관리합니다. 배경은 `#24252B`, 그룹은 `#2C2E38`, 입력칸은 `#373A47`, 강조선은 `#B3ACEE`이며 본문 13픽셀, 그룹 제목 14픽셀, 입력칸 최소 높이 28픽셀입니다. 기존 기본 그룹 색은 스타일시트의 강조색을 따르고 별도로 지정한 그룹 색은 유지합니다. 데이터 작업 공간에 삽입할 때는 `ApplyEmbeddedInspector`가 전용 클래스만 제거하여 편집기 창의 색상과 32픽셀 크기를 사용합니다.

읽기 전용, 드롭다운, Boolean 조건, 열거형 조건 속성 편집기는 UI Toolkit 필드를 제공하여 같은 스타일을 상속합니다. 드롭다운은 직렬화된 값의 변경을 추적하고 조건부 필드는 조건 변경 시 표시 상태를 갱신합니다. 기존 IMGUI 호출 경로도 유지합니다. 하위 타입 선택기 등 별도의 IMGUI 전용 속성 편집기는 기존 렌더링 경로를 사용합니다.

기존 `SWUtilsEditor.uss`는 데이터 작업 공간의 배치만 담당하고, 공통 버튼·입력 필드·팝업·탭·목록 색상은 공통 테마에서 확장합니다. 새로운 IMGUI 탭은 `SWEditorUtils.DrawTabBar`를 사용합니다. 왼쪽 탐색 영역에는 각 창의 이름을 표시하며, 단일 기능 창은 왼쪽 패널 없이 전체 너비를 사용합니다. 데이터 편집기 열기 버튼은 상단에 하나만 둡니다. 긴 제목은 실제 작업 영역 너비에 맞춰 줄을 바꿉니다.

좁은 창에서는 본문의 최소 너비를 우선 확보하고 탐색 탭을 여러 줄로 표시합니다. 안내문은 `SWEditorUtils.DrawHelpBox`, 긴 설명은 `WrappedLabelStyle` 또는 `WrappedSecondaryLabelStyle`로 그립니다. 이 스타일은 테마 범위가 종료된 뒤 Unity가 레이아웃 높이를 계산할 때도 글자 크기를 유지하여 문구가 잘리는 문제를 방지합니다.
